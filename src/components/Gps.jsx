...
const Gps = ({ onCityUpdate }) => {  // Add onCityUpdate prop here
  ...
  const fetchCityName = async (latitude, longitude) => {
    try {
      ...
      // Pass the city name to the parent component
      if (onCityUpdate) {
      ...
      // Pass the fallback city name to the parent component
      if (onCityUpdate) {
        onCityUpdate("Unknown Location");
      }
    }
  };
  ...
  useEffect(() => {
    ...
    getLocation(); // Fetch location and weather data immediately
    const interval = setInterval(getLocation, 30000); // Refresh every 30 seconds
